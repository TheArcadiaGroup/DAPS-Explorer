...
getmaxmasternodecount = async (currentmasternodecount) => {
    result = await client.get('masternodecount');
    let arr = [];
    if (result != null)
        arr = JSON.parse(result)
    if (arr.length > settings.masternodecachecount)
        arr.shift()
    arr.push(currentmasternodecount)
    await client.set('masternodecount', JSON.stringify(arr));
...
updateDb = async (numToUpdate = 0, log = true) => {
    ...
    try { 
        ...
        maxmasternodecount = await getmaxmasternodecount(masternodeconnections.total);
        
        // let difficulty = await (await fetch(hostUrl + 'api/getdifficulty')).json()
        let coinstats = {};
        Object.assign(coinstats, {
            connections: connections,
            masternodeconnections: maxmasternodecount,
