...
dapsapi.get('*', async (req, res, next) => {
  ...
  try {
    ...
    if (reqcollection == "coinstats") {
      await collections[reqcollection].find({}, function(err, result) {
        if (err) {
          console.error(err); res.json({ status: 500, error: err, query: queryObj, collection: reqcollection });
        } else {
          res.json({ status: 200, data: result.length > 0 ? result[0] : {}, query: queryObj, collection: reqcollection })
        }
      });
    }
